...
tags:
  ...
  - Workflow
  - DigitalGarden
description: 作者介绍如何将博客文章同步到微信公众号时保持样式一致，无需重新编辑。通过MacDown配置博客样式，包括标题、正文、高亮、引用等元素的颜色、字体、大小、间距等具体参数，并分享了完整的CSS代码和样式预览图。提供了一套完整的微信公众号样式配置方案，帮助用户快速实现博客样式在微信平台的统一应用。
---

在自己写博客的时候有一些文章会同步到微信公众号，但是又不想重新编辑样式，所以我会直接用 `markdown` 的内容复制到微信里面。
