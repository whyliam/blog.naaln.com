...
tags:
  ...
  - 协同过滤
description: 
  内容型产品在早期通过热度算法冷启动积累用户和内容；热度分由初始热度、用户交互分和时间衰减组成，初始热度按类别和热词设定，用户交互分按点击、收藏、评论、分享配权，时间衰减采用指数函数。规模提升后引入基于内容的推荐，以分词、TF-IDF生成新闻特征向量，用余弦相似度计算新闻关联并构建用户特征实现个性化；进一步扩展至基于用户的协同过滤，根据用户行为和外部画像划分群体，用欧氏距离找相似用户并推荐未读内容。
---
