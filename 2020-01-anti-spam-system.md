...
tags:
  ...
  - DataDriven
  - CommunityGovernance
description: 日课APP反垃圾系统通过设备安全中心、行为监控中心、内容安全中心、举报中心和数据中心五个子系统构建完整防护体系。系统将spam内容分为内容、用户和行为三类，定义违背核心价值观的内容为spam但保持适度包容。系统目标是通过深度学习技术识别色情、推广、辱骂、违禁、涉政、灌水等垃圾内容，结合用户举报机制缩短攻击被检测时间并延长防御控制时间，确保社区内容质量和用户体验。
---
...
但是对于 spam 程度的包容性相对较高。「日课 APP」本身就会聚了各个年代、各个阶层、各个国籍、各个文化程度的内容。如 莫言的《丰乳肥臀》，王小波的《黄金时代》就涉及了大量的色情描写，在用户的回复就不会一棍子打死说一点都不允许色情，
...
用户类指的是使用 spam 信息的个人头像、昵称、个人简介等信息。这样用户可以选择发送正常的动态或评论内容，以此绕过内容上的检查和限制，往往这样的 spammer 会比内容类的更加谨慎和隐蔽。
