...
tags:
  ...
  - Observation
  - Privacy
description: 近期微信朋友圈语音功能火爆网络，作者调查发现该功能并非腾讯官方发布，而是名为微裂变的第三方网站通过微擎开源系统模拟实现。该网站使用微信开放平台SDK的录音播放、隐藏菜单、扫码分享等功能制作出逼真的朋友圈语音界面，具备自动播放、分享传播等特性。虽然模拟度高，但需要在第三方页面收听语音，体验较差且存在信息安全风险。
---
...
后来发现 他只是一个网页的链接，定向到一个 `http://voice.wx7plus.com/wx` 的网站。该网站是一个名为 微裂变 的网站，具体不知道还有什么其他功能。并且採用的是一套名为 [微擎](http://www.we7.cc) 的开源系统。

如我们所知，微信的朋友圈的信息传递一般侷限于朋友之间。但是这种方式发起的 朋友圈语音 很容易造成信息的泄漏，如这条信息是朋友发，但是我可以分享给所有人。[链接](http://www.we7.cc)
...
它使用了微信公共帐号一些简单的 [微信 SDK](http://mp.weixin.qq.com/wiki/home/index.html)
