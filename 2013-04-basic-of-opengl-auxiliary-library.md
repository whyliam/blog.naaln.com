...
- 光照
- C++
description: OpenGL辅助库使用完整指南，详细介绍aux库提供的六类核心函数。OpenGL作为独立于窗口系统的图形API，辅助库解决了初学者创建完整图形程序的困难。内容涵盖：1）窗口初始化和退出——auxInitWindow（创建窗口）、auxInitDisplayMode（设置显示模式）、auxInitPosition（设置窗口位置大小）；2）窗口处理和事件输入——auxReshapeFunc（窗口reshape回调）、auxKeyFunction（键盘响应）、auxMouseFunc（鼠标响应）；3）颜色表装入——auxSetOneColor（设置自定义颜色索引）；4）三维物体绘制——提供球、立方体、长方体、环形圆纹面、圆柱、二十面体、八面体、四面体、十二面体、圆锥、茶壶等10种几何体的网状体和实心体绘制函数；5）背景过程管理——auxIdleFunc（空闲状态执行函数）；6）程序运行——auxMainLoop（场景绘制循环）。辅助库大大简化了OpenGL入门难度，帮助初学者快速掌握三维图形技术。
---
