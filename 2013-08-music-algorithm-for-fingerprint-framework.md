...
- 论文解析
- 音乐识别
description: 音乐指纹识别算法深度解析，对比Shazam和Echoprint两大主流算法框架。文章首先介绍音乐指纹的基本概念：找到能代表曲子的特征，如指纹代表个人一样。详细解析Shazam算法：1）指纹生成——从生谱图生成星座图，取时间点上的最高能量频率，将点与target area内点配对生成hash表；2）匹配选择——手机捕捉音频同样方法做指纹，与服务器数据比较，在坐标系中找斜线判断匹配。深入分析Echoprint算法：1）指纹生成——从声谱图找到八个频段高能点，使用IOI（inter-onset-interval）概念，通过变化阈值和硬性规定长度控制相邻点距离；2）匹配选择——服务端样本拆成60秒片段（50%重叠），直方图记录hash匹配次数选取累计次数最多的值。文章提供大量技术细节和图表，适合音频信号处理和音乐信息检索研究者参考。
---
