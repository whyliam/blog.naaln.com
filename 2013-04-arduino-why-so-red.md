...
tags:
  ...
  - Ecosystem
  - SystemDesign
description: Arduino 是基于 AVR 单片机的开源硬件与开发环境，旨在降低嵌入式学习门槛。板卡集成稳压、USB‑串口等常用电路，提供 Uno、Mega、Nano 等不同尺寸与资源型号。软件采用 Sketch 结构，仅需 setup 与 loop 两函数即可上手。社区提供丰富第三方库与扩展模块，使创意项目快速实现，适用于教学、艺术创作及各类 DIY 场景。
---
...
开放性：Arduino 的硬件电路和软件开发环境都是完全开源的，在不从事商业用途的情况下任何人都可以使用、修改和分发它。这样不但可以使用户更好的理解 Arduino 的电路原理，更可以根据自己的需要而去修改它。比如由于空间的限制，需要设计异形的电路板；或是将自己的扩展电路与主控制电路设计到一起。
...
发条微博自动给你煮咖啡。是不是已经迫不及待，想要开始你的 Arduino 之旅了？赶快到购物网站上搜一搜 Arduino，然后……是不是被各种不同的 Arduino 控制板搞花了眼？
...
Arduino 的代码语法与 C++ 类似，也包含了类和对象的支持。即便是没有任何代码编写的基础，也可以很快上手。关于 Arduino 的代码编写，无论是 Arduino 的官方网站，还是网络上都有了相当丰富的资料，这里不再赘述。
