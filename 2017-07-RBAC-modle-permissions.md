...
tags:
  ...
  - PermissionModel
  - ProductArchitecture
description: 文章倡导采用成熟的授权模型如RBAC以避免再造轮子。RBAC0通过角色分配权限，用户可聚合多角色权限；RBAC1在角色中引入分层与继承，细化粒度；RBAC2引入约束如静态职责分离SSD与动态职责分离DSD，强化安全策略；RBAC3综合RBAC1与RBAC2；用户组提供群体级授权，简化管理。
---
...
正如牛顿所言，站在巨人的肩膀上才能看的更远。我们不妨去参照已有的比较成熟的权限模型，如：RBAC（Role-Based Access Control）——基于角色的访问控制。我搜集了网上很多关于 RBAC 的资料，大多与如何用数据表实现 RBAC 相关，并不容易理解。所以，我会以产品经理的角度去解析 RBAC 模型，并分别举例如何运用这套已得到验证的成熟模型。
