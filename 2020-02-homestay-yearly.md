...
tags:
  ...
  - UserExperience
  - ProductThinking
description: 如程通过收取年度会员费获取预付现金，向民宿签订包房合约将使用权转让给平台，形成资产证券化的融资模式。平台目前拥有约十万付费用户和千二百间签约房源，但用户需求远大于可入住次数，导致预订需提前且取消费用高。民宿方因淡旺季入住率低而获得固定租金，平台通过流量提升入住率。整体模式存在用户规模与房源数量不匹配、优质民宿参与度低和预订限制等问题。
---
...
如程和民宿签了一年的承包合同，即这一年民宿的使用权只可以给如程。同时，如程预先获取了用户的 880 元现金，将固有资产变成可流动资金。此时，如程获得了一年的流动资金用于企业的发展，而民宿每个月可以获得一个月的定期收入。

### 3.2 隐性限制

用户每次可以约定两晚的民宿，但是多数情况需要提前一个星期到一个月约定。（由于前面提及的用户数量太大。）在这样的前提下，用户需要提前规划好自己的行程，就将一部分临时起意的用户给排除在外，也避免了一部分用户的频繁使用。
