...
沙盒的浏览器中遇到了 CAPTCHA（网上有说可以由用户去接管验证码，但是没有找到 demo）。
...
2. **任务规划器**：根据用户输入的任务请求，拆分成 ToDo List，推测是 Claude 模型，因为这一步至关重要，必须要求模型有很强的推理能力，目前来说 Claude 3.5 Sonnet V1。（正在测试 Claude 3.7，估计也在准备 deepseek R1）
...
当打开比如豆包、Kimi 或者类 Computer Use 等应用，向它发一个问题，需要等它回复完。否则在它回复或者做任务的过程中跟它说话，上一段回复/任务就中断了，和它只能是 A-B-A-B 接力式的对话。本质上， Brower 只是一个和 AI 交互的媒介，它可以是 工具，是容器，但不是 AI 全部。
...
给 AI 配一台电脑，同时也给它配系统权限（接入代码仓库、专业数据查询网站等私有 API），并给予一定的培训。AI 可以自己用这台电脑打开浏览器，做动作去调度工具，再根据工具产生的反馈观察它的动作对真实世界造成的影响，再思考下一步，再做动作，再观察……
...
在 Manus 发布后，很多人好奇它背后有没有用到 MCP。创始人说他们的立项比 MCP 早，目前还没有使用 MCP。
下一篇我们看下 [MCP](https://blog.naaln.com/2025/03/MCP) 是什么？

### CUA

在 Manus 发布在计划，OpenAI 发布了 CUA。不写了，写不动了。

### 快行动

![快行动 架构](https://pics.naaln.com/blog/2023-09-10-11cfc8.jpeg-basicBlog)

遥想当年「快行动」项目，还是有点通用 AI Agent 的影子的，我们换个流程图：
