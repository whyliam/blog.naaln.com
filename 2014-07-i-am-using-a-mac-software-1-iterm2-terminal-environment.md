...
tags:
  ...
  - Engineering
  - Tooling
description: iTerm2是macOS终端模拟器，提供快捷键（⌘数字切换标签⌘方向切换⌘F查找正则⌘D垂直分屏⌘ShiftD水平分屏）、剪贴板历史⌘ShiftH、自动补全⌘；、全局呼出、快照回放等功能，支持256色和xterm优化，兼容性好。
---
...
- `ctrl + u`: 清空当前行。这里要注意，mac 默认的 ctrl+u 为清除当前光标至行首的内容，在 iTerm2 中则直接清除本行。iTerm2 也支持其他常用的操作命令，这里顺带讲下，因为这些命令都是 mac 下非常常用也很好用的：
- `ctrl + a`: 到行首
- `ctrl + e`: 行末
...
- 支持 256 色。方便配置 vi 配色。但是在某些远程服务器上不支持 256 色，则只要在 `Prefences->Profiles->Terminal` 里设置为 xterm。
