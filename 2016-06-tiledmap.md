...
tags:
  ...
  - SystemDesign
  - Visualization
description: 瓦片地图分为栅格和矢量两种，栅格瓦片是将矢量数据渲染为两百五十六像素图片并在客户端用框架显示；矢量瓦片以数据格式分块传输，浏览器端使用画布渲染，可实现旋转伪三维效果；瓦片坐标通过墨卡托投影将世界地图分割为不同层级的两百五十六像素单元，形成瓦片金字塔，每层级的瓦片由上层四个子瓦片组成。
---
...
矢量地图瓦片是一种新颖的模式。地图数据会以 json 等格式分块地传输过去，在浏览器端利用前端框架将地图显示出来。在这个基础上，你必须懂得 canvas。整个展示的过程对点线面的渲染有很严格的规定。总体来说，矢量瓦片的数据传输量少，服务器压力小。方便渲染成为各种各样的地图，所以这也是为什么 Mapbox 首推 Mapbox GL 的原因，腾讯地图使用矢量渲染的原因。而且现在移动端不断利用矢量瓦片的技术，考虑到导航时候，需要将地图旋转成为伪 3D 界面。
