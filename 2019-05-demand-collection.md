...
tags:
  ...
  - DecisionMaking
description: 阐述需求采集的关键在于识别背后逻辑而非表面需求，关注用户为何需要而非他们想要的具体功能；在需求管理中区分面向消费者面向企业等其他角色，考虑圈子竞争，并收集来源场景描述原因验收标准；运用卡诺模型评估需求价值，将电商搜索功能划分为基础型索引分类、期望型文字搜索和兴奋型语音与个性化推荐，以满足不同层次的用户需求。
---

## 需求的采集

当我还是初学者的时候，上司让我做一个新项目的需求调研。其实，我心里已经有了一个假设，我需求的调研的目的当然是在为我的产品找支持的依据。自然，最后的产品是我想要的，而不是用户想要的东西。
...
有可能这个用户是父母让他减肥；有可能是要去相亲，为了自己好看一点；有可能会为了健康的身体……发现没有，不同的原因有不同的解决逻辑。自然如果继续深挖下去，还有更深层次的需求。
...
到底是谁的需求？这个问题寻求的是需求的主语。如果按照最常见的方式区分，那就是 C 端用户（ to customer）、B 端用户（to business），以及其他用户，如图所示：
