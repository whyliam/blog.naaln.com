...
下载 setuptools，解压并到该目录下安装，命令是 `sudo python setup.py install`
...
把 bin 文件放进来，转到 bin 文件所在目录，执行命令：

```bash
sudo cp <文件名> /usr/local/java
```
...
如果遇到 Failed to extract the files，则执行命令：
...
文件夹下会生成一个 HelloWorld.class 文件，再输入命令：
...
1. 首先安装 tokyocabinet，网站：[http://fallabs.com/tokyocabinet/](http://fallabs.com/tokyocabinet/)
...
2. 然后安装 tokyotyrant，网站：[http://fallabs.com/tokyotyrant/](http://fallabs.com/tokyotyrant/)
...
然后查看路径/usr/ttserver/casket.tch，这时候就有这个文件了，然后重启一下。
...
我按照 echoprint-server 的 readme 文件中的说法，用 ttservctl start 命令开启 ttserver，这时候 ttserver 的写入数据库是写入到/var/ttserver/casket.tch 中，开启后在命令行输入命令：
...
```
error while loading shared libraries: libtokyocabinet.so.8: cannot open share object file: No such file or directory.
```
...
ln -s /usr/local/tokyotyrant-1.4.41/lib/libtokyocabinet.so.8 /usr/lib/
```
