...
tags:
  ...
  - Workflow
  - SystemsThinking
description: 文章系统阐述了产品设计的六步流程：先列信息结构图梳理数据；再绘产品结构图组织功能；随后以线框图进行原型设计；引入用例模型明确交互流程；补充功能流程图展示逻辑；最后撰写产品需求文档完成需求。文中也对比了信息结构、功能结构与产品结构的作用与适用场景。
---
...
> 当到了原型设计这一步时，已经不仅仅是构思了，我们需要更加深入的了解每个页面上的元素和这些元素的属性。例如按钮元素，我们就需要考虑这个按钮的功能，并且这个功能操作后带给后端和前端的反馈。例如注册会员按钮，用户操作后，第一步逻辑是验证用户输入的信息是否合法，不合法则给出前端反馈；合法则和后端通信验证是否已经存在同样信息，已经存在则给出前端反馈，不存在则进入下一步，注册成功；注册成功后的反馈是跳转页面，还是弹出层提示用户完善资料，这些都是需要更详细的考虑。当然这些更细致的思考是留在需求文档撰写时的，而此时我们需要做的就是把这些元素通过原型表现出来。
