...
tags:
  ...
  - Engineering
  - FirstPrinciples
description: 文章演示在OpenGL中利用参数方程绘制球体的完整实现，包括GLUT环境初始化、坐标系、点坐标矩阵生成、四边形分块绘制以及实心或网格模式的切换，提供C代码并说明关键函数与参数范围。
---
...
![请输入图片描述](https://pics.naaln.com/blog/2022-02-14-de42a7.png-basicBlog)
...
在 init() 函数里，分别对屏幕背景颜色，投影模式，光照，光源位置等进行了设置，并启用纹理，灯光和深度测试；
...
这里绘制的球，其实是个球面。把球面看成是由很多个小的四边形平面构成的，这样就可以通过绘制这些小的四边形平面来构成整个球面。而对于两极部分，需要用三角形将球面两端封起来，如果把这些三角形看作是有两个顶点重合的四边形，问题就得到进一步的简化。因此只需知道球面上一系列点的空间坐标，就可以利用这些点来绘制四边形从而得到整个球面。把整个球面展开，可见球面上这些点构成的是一个矩阵，而这个矩阵中纵横相邻的四个元素的坐标正好构成的就是球面上的一个小平面。
...
via: [http://blog.163.com/tianshi_17th/blog/static/48564189200832992845889/](http://blog.163.com/tianshi_17th/blog/static/48564189200832992845889/)
