...
tags:
  ...
  - DesignSystem
  - DecisionMaking
description: 谷歌MaterialDesign数据可视化设计规范指南，涵盖八种图表类型（时间变化、类别比较、排名、占比、关联、分布、流程、关系）的选择原则，样式设计要点（形状、颜色、线段、文字、图标、坐标轴、图例、注释），交互行为规范（渐进式披露、缩放平移、分页、数据控制、动效、空状态），以及三类仪表板设计方法（分析类、操作类、演示类），为数据密集型UI设计提供完整规范参考。
---
...
分析类仪表板让用户能够研究多组数据并发现趋势。通常，这些仪表板包含能够深入洞察数据的复杂图表。
