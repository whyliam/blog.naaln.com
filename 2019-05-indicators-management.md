...
tags:
  ...
  - onedata
  - facttable
description: 数据仓库建模通过第三范式与维度建模实现。第三范式遵循属性唯一、完全函数依赖及消除传递依赖规则；维度建模以分析需求为导向，采用事实表与维度表构建星形或雪花模式。指标模型基于维度建模的 Bus Matrix定义数据域、业务过程、度量等，指标类型包括事务型指标（衡量业务活动如新增注册数）、存量型指标（统计实体状态如商品总数）及复合型指标（组合业务活动与实体状态如转化率）。OneData 是阿里巴巴的集
---
