...
tags:
  ...
  - ProductThinking
  - Infrastructure
description: 数据仓库是面向主题、集成、稳定、时变的决策支持环境，包含OLTP和OLAP两种处理方式。采用关系建模和维度建模两种数据库设计方法，前者追求数据一致性，后者优化查询性能。事实表和维度表构成维度模型核心结构，ETL流程实现数据从OLTP到数据仓库的转换。企业数据资产包括纸质和电子文件，元数据描述数据结构和规则，数据集满足特定部门需求。
---
...
数据仓库一般是从用户实际需求出发，将不同平台的数据源按设定主题进行划分整合，与传统的面向事务的操作型数据库不同，具有较高的抽象性。面向主题的数据组织方式，就是在较高层次对分析对象数据的一个完整、统一并一致的描述，能完整及统一地刻画各个分析对象所涉及的有关企业的各项数据，以及数据之间的联系。
