...
tags:
  ...
  - DecisionMaking
  - Workflow
description: 需求评审是让干系人提前了解产品，介绍设计背景、业务逻辑及价值。评审前需准备流程图、信息结构图、原型、需求文档，并确保格式统一、命名规范、无错字。评审时先阐述需求背景与价值，随后按模块和场景讲解功能与流程，强调从整体到细节并确认争议点采用五分钟原则记录并在会后讨论，争议多则提前结束重新修改。评审后整理遗留问题、制定行动计划、更新需求文档并安排再次评审。
---
...
面对不同的评审环节，可能有不同的评审重点，但是评审的准备工作和评审的重点都是一致的。
...
按模块简单介绍一下产品的功能模块，不要写入细节。只是让参会人员先有一个概念，产品的结构是怎么样，有什么样的一个优先级。
