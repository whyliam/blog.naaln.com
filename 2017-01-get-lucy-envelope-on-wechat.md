...
tags:
  ...
  - ProductThinking
  - Observation
description: 微信红包的分配在打开时实时计算，金额在零点零一到剩余平均值两倍之间随机，先抢后抢的期望相同但后抢的方差更大；使用提供的Java函数模拟三十人抢五百元两次和二千次，统计结果显示各人获额基本相近，整体分布近似；因此不必过分在意抢红包的时机。
---

### 先说点人话

1. 先抢和后抢，抢到的红包的期望基本相同，就是从概率上来说抢到的钱是差不多的。
2. 但是后抢的红包方差比较大，就是有可能 `0.01`, 也很有可能 `手气最佳`。
