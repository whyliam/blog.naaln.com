...
tags:
  ...
  - Learning
  - Engineering
description: 通过矩阵栈管理状态变换，描述为什么在坐标变换前使用glPushMatrix保存当前矩阵，变换后使用glPopMatrix恢复，保证同一位置绘制不受后续变换影响，并给出移动与旋转后再绘制光位置进行光照的示例。通过嵌套矩阵栈可在不同局部坐标系独立变换物体、摄像机与光源。
---
...
当我们把 glPushMatrix() 和 glPopMatrix()**注释掉**

以后我们发现，当我们再想在同样的位置绘制一个正方形的时候，就会发现已经按我们的 glTranslatef() 所指定的沿 x 轴偏移了 0.2 个单位。而当我们**不把**两句函数调用注释掉时，运行发现，绿色的正方形覆盖了原来的红色的正方形。
