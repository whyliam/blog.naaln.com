...
> CODE：大体上 git 就像大家那么用没有什么问题，只是开发过程中其实发现了一些 git 的问题，比如 git 编码存在问题，没有对 commit log 和 path 做 encoding ，在 Web 显示的时候需要额外的处理。可以参考下 [这篇文章](http://saito.im/note/Drawbacks-of-Git/)。
> git 对大文件大仓库的支持不是很好，当一个项目变大的时候，适时的拆分一下。要不然就要像 Facebook 那样去 hack hg 来支持大仓库。
