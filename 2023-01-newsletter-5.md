...
tags:
  ...
  - LLM
  - SystemsThinking
description: 本文阐述决策的重要性与资源配置、信息依赖、决策时间及置信度阈值的权衡，结合一类二类框架说明决策价值；随后总结人们思维的十七种常见模式，包括部落归属、故事主导、认知局限与脆弱性；最后探讨人工智能技术尤其是大型语言模型如何将笔记转化为第二大脑，自动标签、综合、呈现信息，消除组织难题。
...
首先，他们可以自动标记和链接笔记，无需手动操作。它甚至不需要LLM——现在有一些不太先进、更便宜的模型可以开箱即用。
...
LLM可以真正将您的笔记变成第二个大脑。它们可以在您编写笔记时丰富笔记以创建更多上下文，自动对其进行分类和合成，并以稍后点击的方式将它们呈现给您——这样您就可以实际使用它们。
