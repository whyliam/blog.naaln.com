...
- 代码优化
- 编程规范
description: 代码重构方向指导原则，探讨重构艺术的核心理念。文章阐述了重构的本质：改进软件质量而不改变功能特征，如同散文结构修改。提出两种基本重构方法：归纳方法（分解过程为可重用组件）和内联方法（消除小方法）。文章的核心贡献在于当没有明显代码异味或重构模式时提供的17条实用原则：提炼复用、封装可见度、消除依赖、多态代替switch、组合优于继承、简化构造函数、避免条件表达式、使用常量代替魔幻数字等。强调重构是爬山式的渐进改进，核心问题是如何找到上山的正确道路。适合软件工程师和代码质量追求者参考。
---
