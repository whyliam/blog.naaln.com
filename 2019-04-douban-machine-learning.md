...
tags:
  ...
  - Automation
  - ComputerVision
description: 四年前开发豆瓣害羞组图片爬虫，收集三万张帖子包含各种类型图片。引入OpenNSFW机器学习模型对图片进行筛选，该模型将图片分为绘图、变态、安全、性感、色情五类。安全值大于0.8可确定分类，0.2至0.8需人工介入。提供NSFW图像数据集统计信息，包含159个类别、158.9万个URL，经清洗后约500GB数据量约130万张NSFW图像。
---
...
我陆续爬取了 30000 篇帖子，但是里面什么样的照片都会有，比如黄色，卡通，抠脚大汉…… 之前通过帖子的标题，热度等过滤了一些，但是剩下的图片还是一个海量的工作。
