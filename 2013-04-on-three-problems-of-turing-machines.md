...
tags:
  ...
  - HistoryOfIdeas
  - Knowledge
description: 回答图灵机模型中的三个核心问题：为何存在不可判定问题、强大图灵机为何不停机、图灵设计图灵机的历史背景。通过康托对角线方法证明语言集合不可数而图灵机集合可数，说明存在图灵机无法识别的语言。构造停机问题证明图灵机无法判定自身行为。图灵机设计旨在解决希尔伯特判定问题，但其自身限制最终证明计算本质存在不可逾越的界限。
---
...
图灵机（[Turing machine](http://en.wikipedia.org/wiki/Turing_machine)）是英国数学家阿兰·图灵（[Alan Turing](http://baike.baidu.com/view/2130.htm)）于 1936 年设计的一种抽象机器，用于定义和模拟计算（computing）。图灵机虽然构造简单，但却极其强大，它能模拟现代计算机的所有计算行为，堪称计算的终极机器。然而即便是这个终极机器，也有令它无能为力的问题，这便是第一个要回答的问题：为什么图灵机有不可判的问题？
