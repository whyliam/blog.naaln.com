...
tags:
  ...
  - KnowledgeManagement
  - Life
description: 创业以盈利为核心，聚焦有效产出而非盲目扩张或压低成本，依据制约理论识别系统瓶颈并优化；实践学习、写作、行动和纪律提升个人产出，知识API提供结构化认知模型帮助持续进步；求真兼顾近远事务，理想源于具体事务和长期坚持，在每次选择中转化为对他人的价值。
...
比如决定一个野外队伍行进速度的，不是走的最快的人，而是走的最慢的那一个。这听起来很简单，但实际上现实中却很容易忽略这个事实，出现下面的几个误区：
...
当然，在实践中，抽象的理想会变得具体，但也会被改变。因为这个过程环境和自己相互作用，会导致理想也逐渐发生变化。但多数时候，我们面临的，不是坚持不坚持理想这样的大选择，而是发生在毫厘之间的一个又一个的选择。比如 flomo 要不要融资？要不要每周发一次推送？要不要在开屏放个广告？
